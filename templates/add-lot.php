<?php
$lot_name_saved_values = [];
for ($i = 0; $i < count($required_fields); ++$i) {
    $lot_name_saved_values[$required_fields[$i]] = $_POST[$required_fields[$i]] ?? '';
}
?>
...
<div class="page-wrapper">
  ...
  <main>
    ...
    <form class="form form--add-lot container <?php if(isset($errors)): ?>form--invalid<?php endif; ?>" action="add.php" method="post" enctype="multipart/form-data">
      <h2>Добавление лота</h2>
      <div class="form__container-two">
        <div class="form__item <?php if(isset($errors['lot-name'])): ?> form__item--invalid <?php endif; ?>">
          <label for="lot-name">Наименование <sup>*</sup></label>
          <input id="lot-name" type="text" name="lot-name" placeholder="Введите наименование лота" value = "<?= htmlspecialchars ($lot_name_saved_values['lot-name']); ?>">
          <span class="form__error"><?php if(isset($errors['lot-name'])): ?> <?= $errors['lot-name']; ?> <?php endif; ?></span>
        </div>
      ...
      <div class="form__item form__item--wide <?php if(isset($errors['message'])): ?> form__item--invalid <?php endif; ?>">
        ...
        <textarea id="message" name="message" placeholder="Напишите описание лота" ><?= htmlspecialchars ($lot_name_saved_values['message']); ?></textarea>
        <span class="form__error"><?php if(isset($errors['message'])): ?> <?= $errors['message']; ?> <?php endif; ?></span>
      </div>
...
Добавить
            ...
            <span class="form__error"><?php if(isset($errors['lot-img'])): ?> <?= $errors['lot-img']; ?> <?php endif; ?></span>
        </div>
      </div>
      <div class="form__container-three">
        <div class="form__item form__item--small<?php if(isset($errors['lot-rate'])): ?> form__item--invalid <?php endif; ?>">
          <label for="lot-rate">Начальная цена <sup>*</sup></label>
          <input id="lot-rate" type="text" name="lot-rate" placeholder="0" value = "<?= htmlspecialchars ($lot_name_saved_values['lot-rate']); ?>">
          <span class="form__error"><?php if(isset($errors['lot-rate'])): ?> <?= $errors['lot-rate']; ?> <?php endif; ?></span>
        </div>
        <div class="form__item form__item--small <?php if(isset($errors['lot-step'])): ?> form__item--invalid <?php endif; ?>">
          <label for="lot-step">Шаг ставки <sup>*</sup></label>
          <input id="lot-step" type="text" name="lot-step" placeholder="0" value = "<?= htmlspecialchars ($lot_name_saved_values['lot-step']); ?>">
          <span class="form__error"><?php if(isset($errors['lot-step'])): ?> <?= $errors['lot-step']; ?> <?php endif; ?></span>
        </div>
        <div class="form__item <?php if(isset($errors['lot-date'])): ?> form__item--invalid <?php endif; ?>">
          <label for="lot-date">Дата окончания торгов <sup>*</sup></label>
          <input class="form__input-date" id="lot-date" type="text" name="lot-date" placeholder="Введите дату в формате ГГГГ-ММ-ДД" value = "<?= htmlspecialchars ($lot_name_saved_values['lot-date']); ?>">
          <span class="form__error"><?php if(isset($errors['lot-date'])): ?> <?= $errors['lot-date']; ?> <?php endif; ?></span>
        </div>
      </div>
